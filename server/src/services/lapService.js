...
async function calculateLapStats(teamId) {
  try {
    const team = await Team.findById(teamId).populate('mountain').populate('hill')
    if (!team) {
      return null
    }
    ...
    if (laps.length > 0) {
      const sortedLaps = laps.slice().sort((a, b) => a.startDateTime - b.startDateTime)
      const start = sortedLaps[0].startDateTime
...
export async function createLapAndEmitStats(lapData, io) {
  if (!io) {
    throw new Error('[lapStatsUpdate] Socket.IO instance (io) must be provided to createLapAndEmitStats for live updates.')
  }
  const { teamId, participantId, rfidTagId, startDateTime, endDateTime, lapDuration, lapNumber } = lapData
  try {
    ...
    if (participantId) {
      ...
      if (participant.team?.toString() !== team.id) throw new Error('Participant does not belong to team')
    }
    let rfidTag = null
    if (rfidTagId) {
