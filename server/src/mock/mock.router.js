...
const loadCsv = async (filePath) => {
  try {
    ...
    console.log(`[Mock] Loaded ${jsonArray.length} rows from ${path.basename(filePath)}`)
    return jsonArray
  } catch (error) {
...
const generateFullPassingHistory = async (participantsData, overallResultsData) => {
  const passings = []
  ...
  for (const result of overallResultsData) {
    ...
    if (team && team.mountain) {
      ...
      mountainLapCounts.set(mountainName, (mountainLapCounts.get(mountainName) || 0) + 1)
    }
  ...
  console.log(`[Mock] Generated ${passings.length} simulated passings for ${bibToTeamMap.size} teams`)
  return passings
}
...
const initializeMockData = async () => {
  try {
    ...
    console.log('[Mock] CSV files loaded successfully. Generating passing history...')
    allSimulatedPassings = await generateFullPassingHistory(participants, overallResults)

    console.log(`[Mock] Setting up simulation timer. Total passings: ${allSimulatedPassings.length}`)
    setInterval(() => {
      if (nextPassingIndex < allSimulatedPassings.length) {
        ...
        if (nextPassingIndex % 50 === 0 || nextPassingIndex === allSimulatedPassings.length) {
          const progress = ((nextPassingIndex / allSimulatedPassings.length) * 100).toFixed(1)
          console.log(`[Mock] Progress: ${progress}% (${nextPassingIndex}/${allSimulatedPassings.length} passings released)`)
        }
      }
    }, 2000)
